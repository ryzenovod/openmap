...
var filePath = './app/initData/populations.csv';

module.exports = function (sequelize, onSuccessAll) {

    population.findAll().then(function (data) {
        ...
        var populationFields = ['territoryId', 'valueYear', 'valueCount'];
        ...
        function addValue(line) {
            ...
            if (splitedLine[1].trim()) {
                var data = {};
                _.forEach(populationFields, function(key, value){
                    data[key] = splitedLine[value].trim();
                });
                cntAllValues++;
                values.push(data);
            }
        ...
        function eachValue(item, callback) {
            ...
            /*population.findAll({where: {code: item.code}}).then(function (data) {
            ...
            population.create(item).then(function () {
                cntImportValues++;
                callback();
            });
            //});
